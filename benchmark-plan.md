...
## Запуск субагентом на Haiku

Оркестратор вызывает субагента с промптом примерно такого вида:

> Запусти `python3 scripts/benchmark-ollama.py --runs 3`. Это долгая операция (30–90 минут в зависимости от числа моделей). Когда скрипт завершится:
> 1. Прочитай `benchmark-results/benchmark-results.md`.
> 2. Верни краткое саммари (≤200 слов): топ-3 модели по `quality_score`, самая быстрая из пригодных (score ≥ 70), самая экономная по VRAM из пригодных, список моделей со статусом `SKIPPED` или `UNUSABLE` с причинами, любые предупреждения из секции Warnings.
> 3. Не копируй таблицу целиком — оркестратор сам прочитает файл при необходимости.

**Важно для субагента:**

- Скрипт должен поддерживать long timeout (выставляем Bash `timeout=600000` = 10 мин, но ожидаем несколько часов суммарно — поэтому запускаем в **фоне** (`run_in_background: true`) и поллим.
- Если скрипт падает посреди прогона — промежуточные результаты уже в `benchmark-results.json` (см. п. 6 алгоритма); субагент читает их и сообщает об аварии.
...
- LLM-as-judge оценка содержательности (обсуждено, отложено).
- Прогон на нескольких транскриптах разной длины и тематики (сейчас — один).
- Автоматический `ollama pull` отсутствующих моделей (сейчас — считаем, что модели уже загружены).
